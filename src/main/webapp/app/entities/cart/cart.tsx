  ...
  return (
    <div>
      <h2 id="cart-heading" data-cy="CartHeading">
        <Translate contentKey="shoppingCartApp.cart.home.title">Carts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="shoppingCartApp.cart.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/cart/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="shoppingCartApp.cart.home.createLabel">Create new Cart</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cartList && cartList.length > 0 ? (
            ...
            <tbody>
              {cartList.map((cart, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{cart.quantity}</td>
                  <td>{cart.users ? <Link to={`/users/${cart.users.id}`}>{cart.users.id}</Link> : ''}</td>
                  <td>{cart.menu ? <Link to={`/menu/${cart.menu.id}`}>{cart.menu.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="shoppingCartApp.cart.home.notFound">No Carts found</Translate>
            </div>
          )
        )}
      </div>
