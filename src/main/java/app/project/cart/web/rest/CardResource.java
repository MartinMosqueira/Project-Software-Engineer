...
import app.project.cart.domain.Card;
import app.project.cart.repository.CardRepository;
...
/**
 * REST controller for managing {@link app.project.cart.domain.Card}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Card> result = cardRepository
            ...
            .map(existingCard -> {
                ...
                if (card.getExpiration() != null) {
                    existingCard.setExpiration(card.getExpiration());
                }
                if (card.getCode() != null) {
                    existingCard.setCode(card.getCode());
                }
                if (card.getDni() != null) {
                    existingCard.setDni(card.getDni());
                }
